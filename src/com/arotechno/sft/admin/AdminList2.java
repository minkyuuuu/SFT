...
import com.arotechno.sft.admin.AdminList1.ListType1Item;
...
public class AdminList2 extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		final String[][] user = { {"seum8311", "박봉덕", "66"},
									{"cloud2323", "김지미", "68"},
									{"flower123", "홍길동", "76"},
									{"box12", "이만길", "71"},
									{"sky0210", "김하늘", "74"}};
		...
		final String[][] bigData = { {"C(3)", "D(2)", "C(3)", "B(4)", "B(4)", "D(2)", "C(3)", "63"},
										{"C(3)", "D(2)", "C(3)", "B(4)", "B(4)", "D(2)", "C(3)", "65"},
										{"C(3)", "D(2)", "C(3)", "B(4)", "B(4)", "D(2)", "C(3)", "73"},
										{"C(3)", "D(2)", "C(3)", "B(4)", "B(4)", "D(2)", "C(3)", "68"},
	...
	private class ListType2Adapter extends ArrayAdapter<Item> implements View.OnClickListener{
		...
		public View getView(int position, View convertView, ViewGroup parent) {
			...
			if (item != null) {
				...
				if (name_age != null) {
					if(item.userName.equals("박봉덕")) name_age.setText(item.userName + " 할아버지      " + item.userAge + "세");
					if(item.userName.equals("김지미")) name_age.setText(item.userName + " 할머니      " + item.userAge + "세");
					if(item.userName.equals("홍길동")) name_age.setText(item.userName + " 할아버지      " + item.userAge + "세");
					if(item.userName.equals("이만길")) name_age.setText(item.userName + " 할아버지      " + item.userAge + "세");
					if(item.userName.equals("김하늘")) name_age.setText(item.userName + " 할머니      " + item.userAge + "세");
				}
